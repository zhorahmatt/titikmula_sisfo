                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                Terdapat : {{$user->count()}} user dalam pangkalan data ini.<br/>
                                @if ($user->count() != 0)
                                    @foreach ($user as $key => $thisUser)
                                        <tr>
                                            <th scope="row">{{$key+1}}</th>
                                            <td>{{ $thisUser->name }}</td>
                                            <td>{{ $thisUser->email }}</td>
                                            <td>
                                                <a href="{{ route('user.edit',['id' => $thisUser->id]) }}">Ubah</a> ||
                                                <a href="{{ route('user.delete',['id' => $thisUser->id]) }}">Hapus</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
